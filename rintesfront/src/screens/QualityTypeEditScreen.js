...
const reducer = (state, action )=>{
    switch(action.type){
        ...
        case 'FETCH_SUCCESS':
            return {...state, 
                loading: false, qualityType: action.payload};
...
export default function QualityTypeEditScreen() {
    const navigate = useNavigate();
    const { state } = useContext(Store);
    const { userInfo } = state;

    const params = useParams();
    const {id: qualitytypeId} = params;
    ...
    useEffect(()=>{
        const fetchData = async()=>{
            try{
                ...
                const {data} = await axios.get(`/api/qualitytype/${qualitytypeId}`,{headers: {Authorization: `Bearer ${userInfo.token}`}});
                setName(data.name);
    ...
    const submitHandler = async (e)=>{
        ...
        try{
            ...
            await axios.put(`/api/qualitytype/${qualitytypeId}`,{
                name,
                description,
                isActive
            }, {
            ...
            toast.success('Qualidade do produto Actualizado com Sucesso');
...
return (
   <Container className='small-container'>
    <Helmet>
        <title>Editar Qualidade do Produto </title>
    </Helmet>
    <h1> Editar Qualidade do Produto</h1>
