...
export default function ProvinceListScreen() {
  const [{ loading, error, provinces, loadingDelete, successDelete, pages }, dispatch] =
    useReducer(reducer, {
      loading: true,
      error: '',
    });
  ...
  if(window.confirm('Tem a certeza que deseja criar uma nova Província?')){
    navigate('/province/create')
}
}

  const deleteHandler = async (id) => {
    if (window.confirm('Tem a certeza que deseja remover esta Província?')) {
      try {
        dispatch({ type: 'DELETE_REQUEST' });
        const { data } = await axios.delete(`/api/provinces/${id}`, {
 ...
 {[...Array(pages).keys()].map((x)=>(
     <Link className={x + 1 === Number(page)? 'btn text-bold': 'btn'} key={x+1} to={`/provinceList?page=${x+1}`}>
         {x+1}
     </Link>
 ))}
</div>
