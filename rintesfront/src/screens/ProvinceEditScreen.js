...
const reducer = (state, action )=>{
    switch(action.type){
        ...
        case 'FETCH_SUCCESS':
            return {...state, 
                loading: false, province: action.payload};
...
export default function ProvinceEditScreen() {
    ...
    useEffect(()=>{
        const fetchData = async()=>{
            try{
                ...
                const {data} = await axios.get(`/api/provinces/${provinceId}`,{headers: {Authorization: `Bearer ${userInfo.token}`}});
                setName(data.name);
    ...
    const submitHandler = async (e)=>{
        ...
        try{
            ...
            await axios.put(`/api/provinces/${provinceId}`,{
                name,
                isActive
            }, {
            ...
            toast.success('Província Actualizada com Sucesso');
            navigate('/provinceList/');
